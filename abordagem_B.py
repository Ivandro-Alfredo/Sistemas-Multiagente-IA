...
class Agente:
    ...
    def atualizar_estado(self, ambiente):
        ...
        todas_celulas_visitadas = all((x, y) in agente.visitadas 
                                      for x in range(ambiente.tamanho) 
                                      for y in range(ambiente.tamanho) 
                                      for agente in ambiente.agentes)
        
        # Verificar se pelo menos um agente ainda está vivo
        agente_vivo = any(agente for agente in ambiente.agentes if agente.forca > 0)
        # Verificar se todos os agentes morreram
        todos_agentes_mortos = all(agente for agente in ambiente.agentes if agente.forca == 0)
        
        # Verificar se todos os requisitos foram atendidos para encerrar a simulação
        if todas_celulas_visitadas and agente_vivo:
            print("Ambiente totalmente explorado e pelo menos um agente permanece vivo. Simulação encerrada com sucesso.")
...
class Render_Class_B: 
    def renderizar_Abordagem_B():
        ambiente = Ambiente(tamanho=10, probabilidade_livre=0, probabilidade_bomba=0, probabilidade_tesouro=0)
        print('=================== EXCUTANDO ABORDAGEM--B ===================')
        print('==============================================================')
        print('Considerar sucesso se o ambiente for totalmente explorado e pelo menos um agente permanecer vivo.')
