...
class Ambiente:
    def __init__(self, tamanho, probabilidade_livre, probabilidade_bomba, probabilidade_tesouro):
        ...
        # Adicione a inicialização da bandeira
        self.bandeira_pos = (random.randint(0, tamanho - 1), random.randint(0, tamanho - 1))
        self.grid[self.bandeira_pos[0]][self.bandeira_pos[1]] = 'Flag' 
    ...
    def mensagem(self):
        ...
        # Verificando se a bandeira foi encontrada
        if self.bandeira_encontrada:
            print("Bandeira foi encontrada!")
        else:
            print("Bandeira não foi encontrada.")
    ...
    def atualizar_estado(self, ambiente):
        ...
        if celula_atual == 'T':
            ...
            if len(ambiente.agentes)==0:
                print("Nenhum agente encontrou a bandeira. Simulação encerrada.")
                ambiente.encerrar_simulacao()

        elif celula_atual == 'Flag':  # 'F' representa a bandeira
            print(f"Agente {self.nome} encontrou a bandeira!")
            self.bandeira_encontrada = True
            ambiente.bandeira_encontrada = True
            if any(agente.bandeira_encontrada for agente in ambiente.agentes):
                print("Pelo menos um agente encontrou a bandeira. Sucesso!")
                ambiente.encerrar_simulacao()
...
class Modelo_knn:
    ...
    def run():
        ...
        while ambiente.simulacao_ativa:
            for agente in agentes:
                ambiente.start_time = time.time()  
                acao_escolhida = None  # Define acao_escolhida como None fora do bloco try-except
                try:
                    acao_escolhida = agente.explorar_inteligentemente(ambiente)
                except TypeError:
                    pass
...
class Render_Class_C: 
    def renderizar_Abordagem_C():
        ambiente = Ambiente(tamanho=10, probabilidade_livre=0, probabilidade_bomba=0, probabilidade_tesouro=0)
        print('================== EXCUTANDO ABORDAGEM--C ====================')
        print('==============================================================')
        print('Considerar sucesso se pelo menos um agente encontrar a Bandeira. Para tal, deve introduzir a bandeira numa posição aleatória do ambiente.')
